<template>
    <Modal size="lg">
        ...
        <p class="modal-text">
            Edit atau hapus pengumuman sesuai data yang benar
        </p>
            ...
            <div class="detail-grid two-col">
                ...
                <div class="form-group" style="grid-column: 1 / -1">
                    ...
                    <input
                        ...
                        v-model="form.judul"
                        class="form-input"
                        placeholder="Contoh: Informasi Operasional"
                        required
                    />
                ...
                <div class="form-group" style="grid-column: 1 / -1">
                    ...
                    <input
                        ...
                        class="form-input"
                        placeholder="Contoh: Pengumuman berlaku sampai akhir bulan..."
                    />
...
export default {
    ...
    data() {
        return {
            ...
            form: {
                ...
                slug: this.pengumuman?.slug ?? '',
                ringkasan: this.pengumuman?.ringkasan ?? '',
                isi: this.pengumuman?.isi ?? '',

                diutamakan: this.pengumuman?.diutamakan ?? false,
                aktif: this.pengumuman?.aktif ?? true,

                tanggal_publish: this.toDatetimeLocal(
                    this.pengumuman?.tanggal_publish,
                ),
                tanggal_berakhir: this.toDatetimeLocal(
                    this.pengumuman?.tanggal_berakhir,
                ),

                urutan: this.pengumuman?.urutan ?? 0,
            },
        };
    },
    ...
    methods: {
        ...
        submitPengumuman() {
            ...
            router.put(
                `/beranda/pengumuman/update/${this.pengumuman.id}`,
                this.form,
                {
                    onSuccess: () => {
                        triggerAlert('success', 'Pengumuman berhasil diupdate');
        ...
        deletePengumuman() {
            ...
            // ✅ route delete kamu menggunakan POST /delete/{id}
            router.post(
                `/beranda/pengumuman/delete/${this.pengumuman.id}`,
                {},
                {
                    onSuccess: () => {
                        triggerAlert('success', 'Pengumuman berhasil dihapus');
                        this.$emit('closeModal');
                        this.$emit('refreshData');
                    },

                    onError: () => {
                        triggerAlert('error', 'Gagal menghapus pengumuman');
                    },
        ...
        toDatetimeLocal(val) {
            ...
            // kalau sudah "2026-01-25T10:38" -> aman
            if (val.includes('T')) return val.slice(0, 16);

            // kalau "2026-01-25 10:38:00" -> ubah jadi datetime-local
